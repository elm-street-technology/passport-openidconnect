...
Strategy.prototype.authenticate = function(req, options) {
  ...
  var tokenURL = options.tokenURL || this._tokenURL;
  var userInfoURL = options.userInfoURL || this._userInfoURL;
  ...
  if (req.query && req.query.code) {
    ...
    oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
      ...
      self._shouldLoadUserProfile(options, iss, sub, function(err, load, options) {
        if (err) { return self.error(err); };

        if (load) {
          var userInfoURL = self._userInfoURL;
          if (options && options.userInfoURL) {
            userInfoURL = options.userInfoURL;
          }
          var parsed = url.parse(userInfoURL, true);
        ...
        function onProfileLoaded(profile) {
          ...
          self._verify(req, profile, idToken, verified);
        }
      });
    });
  } else {
    ...
    var location = authorizationURL + '?' + querystring.stringify(params);
...
/**
 * Check if should load user profile, contingent upon options.z
 *
 * @param {String} issuer
 ...
 * @api private
 */
Strategy.prototype._shouldLoadUserProfile = function(options, issuer, subject, done) {
  if (typeof this._skipUserProfile == 'function' && this._skipUserProfile.length > 1) {
    ...
    if (!skip) { return done(null, true, options); }
    return done(null, false);
  }
}
